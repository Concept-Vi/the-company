...
F8 affordances that ARE in scope — CC-24.1, CC-25.1/.2/.3, CC-33.1/.2/.3/.4, CC-35.1 — are contracted
`building`/`planned` in their entries, not here.) Distinction held per CONTRACT-FORMAT §4.2 / §1
...
| CC-24.2 / CC-24.3 / CC-24.4 | switch account, supply/change a credential, mint a CI token | changing Anthropic credentials is intrinsically a HOST act (/login, /logout, env, `claude setup-token`) AND a usage-policy boundary (third parties may not route through another user's plan creds) | the recovery is the host path, named in [[auth#op: auth.act]]; the company inherits the service account's credential | "re-authenticate", "switch accounts", "log out", "generate auth token" | F8 lane (auth.md) | 2026-06-12 |
