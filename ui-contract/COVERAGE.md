...
captured: 2026-06-12
status: MACHINE-RENDERED headline (tools/coverage.py --render-md owns the machine block; GROUND_TRUTH
  re-armed 2026-06-13 after the Session Fabric R1.4 wrapper REMOVAL — the ③④⑤ command-wrapper layer
  was deleted as wrong-approach, so wrapper-only ops flipped building→planned honestly: 13 covered /
  22 planned-only classes, 45/78 affordances). Layer-1 (CONTRACTED) only; Layer-2 (DEMONSTRATED —
  task verdicts / load.jsonl) is empty because no driving harness has run (no op is `live`).
...
|---|---|---|
| **COVERED** (≥1 affordance reached by a `building`/`live` op — real cited endpoint) | **13** | CC-05, CC-07, CC-08, CC-09, CC-10, CC-14, CC-15, CC-18, CC-20, CC-23, CC-25, CC-33, CC-35 |
| **PLANNED-ONLY** (mapped, but EVERY covering op is `planned` — no real endpoint yet) | **22** | CC-01, CC-02, CC-03, CC-04, CC-06, CC-11, CC-12, CC-13, CC-16, CC-17, CC-19, CC-21, CC-22, CC-24, CC-26, CC-27, CC-28, CC-29, CC-30, CC-31, CC-32, CC-34 |
| **UNMAPPED** (no affordance seeded, no op — starvation) | **0** | — (every class opened) |
| total | **35** | — |

Affordance grain (the finer instrument): **123 affordances defined in FEATURE-ATLAS, 123 reached by ≥1 op** (zero silent gaps, zero phantom tags — closure both directions). Of the 123 reached: **45 via a `building`/`live` op** (a real cited endpoint), **78 via `planned` ops only**.
...
> **CORRECTED (Session Fabric R1.4, 2026-06-13):** the machine block above is the truth surface —
> **22 planned-only CLASSES, 78 planned-only affordances**. The 2026-06-12 L-Wire notice that stood here
> (claiming CC-03/04/06/11/12/13/16/17/19/21/22/24/26/27/29/30/32/34 flipped `building` behind the ③④⑤
> MCP tools + `/api/{config,dev,auto}/*` arms) is REVERSED: that wrapper layer was REMOVED as
> wrong-approach (it duplicated native session ability — the operational-requirements honest ledger), so
> those classes are PLANNED-ONLY again and the per-class detail below is directionally accurate once more.
> The capabilities are reached by DRIVING A REAL SESSION (supervisor + R1-prime + wire); a class flips
> back to covered only when a drive-real-session op cites a real endpoint — never via a rebuilt wrapper.

These classes were MAPPED (each has ≥1 entry contracting the native data model); the rows below split
into two reasons — held apart, never conflated (CONTRACT-FORMAT §4.2). The machine block's count
(22 planned-only classes) is the truth surface:
...
the CC-04/CC-29/CC-32/CC-34 prose below still reads OUT-OF-LOCAL-SCOPE but the ③ config lane reopened
them as `building` R3 config-face ops — that prose is stale and is the housekeeping lane's to refresh.)
...
78 affordances are `planned`-only (machine join, re-rendered 2026-06-13 after the Session Fabric R1.4
wrapper removal — the ③④⑤ lanes' building flips were REVERSED when that layer was deleted as
wrong-approach; the drive-real-session fabric is the build path now). The buildable subset (excluding the out-of-local-scope boundaries) is
...
- **OUT-OF-SCOPE.md affordance-grain exclusions** (the genuinely-inert F8 host/org boundaries:
  CC-01.1/02.1, CC-28.1, CC-31.1 — drive-the-TUI / org-admin / monorepo-setup, which a headless
  fabric truly cannot do). Each names what it would be, why out, what to do instead, aliases,
  decided-by, date — never silent. CC-24.2/.3/.4 was REMOVED from the exclusion list (reopened as
  `building` consent-gated R3 acts, Tim's sole-operator steer — OUT-OF-SCOPE.md carries the reopening
  note). CC-04.1/CC-29.1/CC-32.1/CC-34.1 are still listed as exclusions in OUT-OF-SCOPE.md but the ③
  config lane reopened them as `building` config-face R3 ops — those exclusion rows are STALE and are
  the housekeeping lane's to reconcile (flagged, never silent). CLEAR for CC-24; FLAGGED for CC-04/29/32/34.
...
— the authoritative counts are the machine block (regenerated by `tools/coverage.py --render-md` from
the live join): **13 COVERED / 22 PLANNED-ONLY / 0 UNMAPPED classes · 123/123 affordances · 45 building /
78 planned-only** (2026-06-13, after the Session Fabric R1.4 wrapper removal — the kept fabric covers;
wrapper-only coverage honestly reverted to planned).
