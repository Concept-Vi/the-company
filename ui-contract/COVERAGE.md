...
Affordance grain (the finer instrument): **123 affordances defined in FEATURE-ATLAS, 123 reached by ≥1 op** (zero silent gaps, zero phantom tags — closure both directions). Of the 123 reached: **96 via a `building`/`live` op** (a real cited endpoint), **27 via `planned` ops only**.
...
> **STALE-PROSE NOTICE (Capability Fabric L-Wire, 2026-06-12):** the machine block above is the truth
> surface — **4 planned-only CLASSES (CC-01/02/28/31), 27 planned-only affordances**. The per-class
> table and reasons in THIS section predate the ③④⑤ face+wire lanes: most rows below (CC-03/04/06/11/12/
> 13/16/17/19/20/21/22/24/26/27/29/30/32/34) have since flipped to `building` (a real MCP-face op + the
> now-landed `/api/{config,dev,auto}/*` bridge arm — §8). The DETAIL/reasoning below is preserved for the
> backend-gap map; the COUNTS in it are superseded by the machine block. Full per-row reconciliation is a
> follow-on housekeeping pass — flagged loud, never silently left as truth.

These classes were MAPPED (each has ≥1 entry contracting the native data model); the rows below split
into two reasons — held apart, never conflated (CONTRACT-FORMAT §4.2). NOTE the count is the machine
block's (4 planned-only classes), not the "24" this prose was authored against:
...
27 affordances are `planned`-only (machine join, 2026-06-12 — down from the 91 this prose was first
authored against, as the ③④⑤ lanes flipped their ops to `building`). The buildable subset (excluding the out-of-local-scope boundaries) is
...
the live join): **31 COVERED / 4 PLANNED-ONLY / 0 UNMAPPED classes · 123/123 affordances · 96 building /
27 planned-only** (2026-06-12, after the ③④⑤ Capability Fabric face+wire lanes landed).
