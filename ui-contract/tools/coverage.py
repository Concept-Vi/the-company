...
def print_map(cov, data, diffs):
    ...
    print("-" * 78)
    if not diffs:
        print("MATCH — machine join reproduces the corroboration map exactly:")
        print(f"  {len(cov['covered'])} covered / {len(cov['planned_only'])} planned-only / "
              f"{len(cov['unmapped'])} unmapped · {len(cov['reached'])}/{len(data['defined_affordances'])} "
              f"affordances · {len(cov['aff_building'])} building / {len(cov['aff_planned_only'])} planned-only.")
        print("  The machine join corroborates the GROUND_TRUTH anchor. No divergence.")
    else:
...
# ─────────────────────────────────────────────────────────────────────────────
# --render-md : regenerate ONLY the machine-derived tables in COVERAGE.md, in place,
# PRESERVING every human-authored prose section. The machine block is delimited by HTML-comment
# SENTINELS so a regen replaces exactly that block and nothing else (deterministic: same join →
# byte-identical block; run twice → identical file). On the FIRST run (no sentinels yet) the block
# is inserted right after the "## Headline counts" heading, wrapping the existing hand-derived
# headline table+prose so subsequent runs own ONLY the machine numbers — the per-class affordance
# tables, the PLANNED-ONLY narrative, the flagged lists, and Layer-2 prose are never touched.
# ─────────────────────────────────────────────────────────────────────────────
MACHINE_BEGIN = "<!-- COVERAGE:MACHINE-BEGIN (generated by tools/coverage.py --render-md; do not hand-edit this block) -->"
MACHINE_END = "<!-- COVERAGE:MACHINE-END -->"
HEADLINE_HEADING = "## Headline counts (class grain — the F9.3 unit)"


def _fmt_class_list(classes):
    """A compact, DETERMINISTIC class list (sorted) for a table cell."""
    return ", ".join(sorted(classes)) if classes else "—"


def render_machine_block(cov, data) -> str:
    """Build the machine-derived block (between the sentinels) from the live join. Pure + deterministic
    — sorts every set, no timestamps, no dict-order dependence — so two runs over one corpus produce a
    byte-identical block (the regen-safety property the build relies on)."""
    n_classes = len(data["defined_classes"])
    n_cov = len(cov["covered"])
    n_planned = len(cov["planned_only"])
    n_unmapped = len(cov["unmapped"])
    n_aff_def = len(data["defined_affordances"])
    n_aff_reached = len(cov["reached"])
    n_aff_building = len(cov["aff_building"])
    n_aff_planned = len(cov["aff_planned_only"])
    sg = cov["defined_not_used"]
    pt = cov["used_not_defined"]
    lines = [
        MACHINE_BEGIN,
        ...
        "|---|---|---|",
        f"| **COVERED** (≥1 affordance reached by a `building`/`live` op — real cited endpoint) "
        f"| **{n_cov}** | {_fmt_class_list(cov['covered'])} |",
        f"| **PLANNED-ONLY** (mapped, but EVERY covering op is `planned` — no real endpoint yet) "
        f"| **{n_planned}** | {_fmt_class_list(cov['planned_only'])} |",
        f"| **UNMAPPED** (no affordance seeded, no op — starvation) "
        f"| **{n_unmapped}** | {_fmt_class_list(cov['unmapped']) if cov['unmapped'] else '— (every class opened)'} |",
        f"| total | **{n_classes}** | — |",
        "",
        f"Affordance grain (the finer instrument): **{n_aff_def} affordances defined in FEATURE-ATLAS, "
        f"{n_aff_reached} reached by ≥1 op** "
        f"({'zero' if not sg else len(sg)} silent gap{'s' if len(sg)!=1 else ''}, "
        f"{'zero' if not pt else len(pt)} phantom tag{'s' if len(pt)!=1 else ''} — closure both "
        f"directions). Of the {n_aff_reached} reached: **{n_aff_building} via a `building`/`live` op** "
        f"(a real cited endpoint), **{n_aff_planned} via `planned` ops only**.",
        "",
        f"_Ops parsed: {data['n_ops']}. Machine-regenerated by `tools/coverage.py --render-md` from the "
        f"live join (resources/*.md `atlas:`+`status:` ↔ atlas/FEATURE-ATLAS.md). This block is the "
        f"only machine-owned region; all prose below is human-authored and preserved._",
        "",
        MACHINE_END,
    ]
    return "\n".join(lines)
...
def rewrite_coverage_md(root: Path, cov, data) -> dict:
    """Surgically replace the sentinel-delimited machine block in COVERAGE.md, preserving all prose.
    Returns {path, action, changed}. Idempotent: if the rendered block equals what's on disk, the file
    is left byte-identical (no churn). FAILS LOUD if COVERAGE.md is missing (never silently creates a
    bare file that would lose the prose map)."""
    path = root / "COVERAGE.md"
    if not path.exists():
        raise SystemExit(f"FATAL: {path} not found — --render-md regenerates the machine tables IN "
                         f"the existing prose map; it never creates one from scratch (the prose is "
                         f"human-authored). Restore COVERAGE.md and re-run.")
    text = path.read_text()
    block = render_machine_block(cov, data)

    if MACHINE_BEGIN in text and MACHINE_END in text:
        # replace exactly the existing block (everything else is untouched)
        pre, rest = text.split(MACHINE_BEGIN, 1)
        _old, post = rest.split(MACHINE_END, 1)
        new_text = pre + block + post
        action = "replaced machine block"
    else:
        # FIRST run: wrap the hand-derived headline section. Insert the block right after the
        # "## Headline counts" heading, BEFORE the existing hand-derived table+prose, and comment the
        # old hand table out of the way is NOT done (no-versioning) — instead we REPLACE the old
        # hand-derived headline body (heading→next "---" or next "## ") with heading + block, since the
        # headline numbers are exactly the machine-owned region. Prose elsewhere is untouched.
        if HEADLINE_HEADING not in text:
            raise SystemExit(f"FATAL: cannot find the '{HEADLINE_HEADING}' heading in COVERAGE.md to "
                             f"anchor the machine block — the prose structure changed; fix the anchor "
                             f"(fail loud, never guess where to insert).")
        head_idx = text.index(HEADLINE_HEADING)
        after_head = head_idx + len(HEADLINE_HEADING)
        # the hand-derived headline body runs to the next horizontal rule "\n---\n" (the section break
        # before "## Layer 1"). Replace that body with the sentinel block; keep the heading + the rule.
        rule = text.find("\n---\n", after_head)
        if rule == -1:
            raise SystemExit("FATAL: no section break '---' after the Headline heading — fix the "
                             "COVERAGE.md structure (fail loud).")
        new_text = text[:after_head] + "\n\n" + block + "\n" + text[rule:]
        action = "inserted machine block (first run; wrapped the headline section)"
...
def main():
    ...
    ap.add_argument("--render-md", action="store_true",
                    help="rewrite ONLY the machine-derived tables in COVERAGE.md (sentinel-delimited), "
                         "preserving all human-authored prose; default is non-destructive stdout")
    args = ap.parse_args()
    ...
    if args.render_md:
        # Regenerate ONLY the sentinel-delimited machine block; all prose is preserved. Deterministic
        # (run twice -> byte-identical). Burns down planned->building automatically as each op flips.
        res = rewrite_coverage_md(root, cov, data)
        print(f"--render-md: {res['action']} in {Path(res['path']).name} "
              f"({'CHANGED' if res['changed'] else 'no change — already current'}).")
