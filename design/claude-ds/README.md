...
- **The Block — the container universal component** (`app/registry/block-type.js` → `CV_BLOCK`, `tokens/material.css` + `tokens/glass.css`, `axes/{material,flow,level,density}/*`, `app/registry/text-type.js`, `app/ai/ai-block.js`): the Glyphic's grammar applied to **containers** — the tangible units of zones that glyphics, typed text, and every primitive live *inside*. A Block = a **ground** part (the plane behind; texture from the full texture axis via `data-ground-texture`) + a **surface** part (**material axis**: glass · velum · parchment · stone · none — glass binds to `tokens/glass.css`, which also owns the graduated zone-TINT whisper `data-glass-zone`) + a **coordinate in depth** (the **level axis**: `page→region→panel→card→well→overlay`; the rung *derives* the material depth-modifier and the spacing rhythm — deeper breathes tighter) + **ordered sockets** (`header→body→footer→primary→actions→onActivate`; order is reading order) + **action sockets** filled by address (`action:<id>`) from `CV_ACTIONS` through the same one mechanic. `CV_BLOCK.chrome(spec)` resolves an instance to `{classes, attrs, style}` of pure system utilities (`.material`, `.keyline`, `.contain`, `.flow`, `arrange` layout); `CV_BLOCK.walk()` assigns `{path, depth}` coordinates — composition IS the coordinate system; pages are made only of resolutions. Typed **text** occupants (`text-type.js`: role → `.t-*` classes + length budgets from `tokens/text.css`) fill content sockets; the **one inspector** edits a block like a glyphic; `block.compose` (`ai-block.js`) generates whole compositions validated against the live axes. Living spec + demo: **`system/block-system.html`** (Design-System tab → "Block System"); architecture: **`analysis/BLOCKS.md`**.
- **Skins — the registry of worlds** (`tokens/skins.css`, `axes/skin/skin-axis.js`, `analysis/SKINS.md`): a skin is a coordinated point across the existing axes (ground × default surface × shadow character × thread/edge language × state glow), scoped by `data-skin`, re-binding ROLE vars only. Material **`skin`** = "the active skin's own surface" (the block default): **glass** (dark, refractive — the default world) or **stone** (champagne plaster + matte porcelain laid on it, per the May-2026 reference boards). Block **states** (`selected` gold rim-glow · `potential` dashed ghost · `stacked` deck) and luminous **threads** (the graph side's connectors) speak each skin's language. Living spec: **`system/skin-system.html`** — one composition, two worlds.
- **Component layer** (`tokens/controls.css` → the `Button` / `Card` / `Input` / `Badge` / `Tabs` / `Segmented` / `Switch` / `Avatar` / `Modal` / `Stepper` / `Popover` / `Tooltip` / `Select` primitives in `components/`): the bread-and-butter controls — buttons (gold action · dark panotour · gold-outline pill · ghost · soft · sage comm), cards (cream / surface / outline / gold / raised), fields (gold-focus inputs, textarea, select, search), badges, stat pills, chips, tags, tables, dividers, plus **tabs** (gold-underline active, the dashboard tab bar), the **segmented audience toggle**, **switches** (gold-on), **avatars**, **tooltips**. Token-pure and theme-aware (every value resolves to a role token incl. `--on-gold` for fixed-gold surfaces, so the same class works on light/dim/dark/clean grounds), grounded in the source's real control vocabulary. The React primitives are thin wrappers over the `cv-*` classes; see the **Component Library** card.
...
├── components/                ← v2 — the UI primitives: Button · Card · Input · Badge · Tabs · Segmented · Switch · Avatar · Modal · Stepper · Popover · Tooltip · Select · Glyphic (the universal compositional mark) — token-pure wrappers over tokens/controls.css + CV_GLYPHIC
