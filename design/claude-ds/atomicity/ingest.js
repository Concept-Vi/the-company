...
For each finding keep detail and evidence under ~22 words. Return AT MOST 6 of the highest-signal findings. Plain JSON, no markdown fence, nothing outside it.`;
  ...
  AI.register({ id: 'source.recognize', name: 'Recognise source', layer: 'capability', family: 'ingest', description: 'Detect an uploaded source’s type and context.', surfaces: ['ingest'], provider: 'claude', icon: 'search', provenance: 'built-in', run: a => recognize(a.params.id) }, { silent: true });
  AI.register({ id: 'source.analyze', name: 'Analyse source deeply', layer: 'capability', family: 'ingest', description: 'Mine an uploaded source for design DNA across colour, type, voice, structure, values and motifs.', surfaces: ['ingest'], provider: 'claude', behaviours: ['voice.conceptv'], icon: 'sparkles', provenance: 'built-in', run: a => analyze(a.params.id) }, { silent: true });
