...
- **Arbitrary self-mod is bounded** — extensions are additive `.tsx` in `src/extensions/` (import only `react`), build-gated by syntax + import-allowlist (NOT a full type-check — `tsc` is too slow on tldraw). In-place rewrites of `App.tsx`/engine internals through the brain are deliberately out (would risk the build). **Operator-approval is governance, NOT a safety guarantee for generated code** — the floors are the build-gate + error boundary + git-revert + single-user machine.
