...
| orpheus | ⏳ install in progress | pending | `orpheus-speech 0.1.0` requires `vllm` (UNPINNED → pulls the latest, multi-GB) + `snac`. Still downloading after ~36 min at report time (venv at base size, vLLM+torch wheels not yet landed) — left running in the background, NOT killed. Resume/finish: `~/.voice-venvs/orpheus/bin/pip install orpheus-speech soundfile` (let it complete); if the resolved vLLM is the buggy build, `~/.voice-venvs/orpheus/bin/pip install vllm==0.7.3` after. Then wrapper import-check: `~/.voice-venvs/orpheus/bin/python -c "import sys; sys.path.insert(0,'/home/tim/company'); from orpheus_tts import OrpheusModel; import voice.engines.orpheus"`. |
