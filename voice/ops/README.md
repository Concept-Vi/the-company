...
| orpheus | ✅ | ✅ lib + wrapper module | `orpheus-speech 0.1.0` + `vllm 0.22.0` + `torch 2.11.0` + `snac 1.2.1` (the install was slow — vLLM is multi-GB — but completed exit 0). `from orpheus_tts import OrpheusModel` + `voice.engines.orpheus` both import clean; 8 voices present (tara/leah/…). Generation unverified (needs GPU; heaviest VRAM — mind co-residency with the chat brain on the 16 GB card). If the resolved vLLM 0.22.0 misbehaves at load, pin `vllm==0.7.3` per REQUIREMENTS.md. |
...
5. **cosyvoice install** — the only engine not import-clean (repo-build blocker; see its row + REQUIREMENTS.md). The other four (qwen3tts/xtts/chatterbox/orpheus) are installed + import-verified.
6. **The UI lane + per-mode voice** (finish-checklist STEP 4) — a separate FORM session.
