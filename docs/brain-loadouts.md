...
- **Switch to the 9B (quality/judging):** `company up @quality-9b --evict` — evicts the interaction loadout (the 9B fills the card) and starts the 9B solo. ⚠️ Today this does NOT auto-repoint the RHM (the [[project-loadout-resolution|atomic loadout-switch]] is the fix-in-progress); until then, after the switch repoint the RHM: `POST /api/rhm-config {"model":"RedHatAI/Qwen3.5-9B-FP8-dynamic","base_url":"http://127.0.0.1:8002/v1"}` (and back to `:8001` when returning to the 4B).
