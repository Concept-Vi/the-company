...
| to **operate the running system** (see / start / stop / restart) or add a **service to the command center** | `ops/` | the `company` console + `ops/services.json` (the registry of what runs). Add a service = add an entry (+ a unit in `ops/systemd/` if needed). **This is the *first* command center — more will follow the same pattern; see `ops/AGENTS.md`.** |
