...
9. **AI-operated is NOT review-free.** Every feature includes a UI/UX review + implementation pass; **review (the operator via the RHM organ + review agents) is MANDATORY and is a SEPARATE stage from implementation.** No implementation is "done" until its result is **surfaced for review** and any operator-facing surface meets the **product bar**; a backend-only change still updates the surface that exposes it. Concretely in the decision→implementation wire: `implemented` means "done AND surfaced for review" (a `decision.surfaced_for_review` event + a review inbox item carrying the result + the changed-files diff), **never a silent terminal**. This is coherent with rule 4 (fail loud — a silent close is a silent failure) and with *reflects-never-owns*: the wire writes the `status` lane and surfaces a review item; it **never** writes the operator `resolved` field. The operator resolves the review.
