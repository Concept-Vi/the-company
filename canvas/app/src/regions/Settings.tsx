...
// endpoint — switchPersona (/api/voice/switch + /api/rhm-config), changeMode (/api/mode), setCfgSlot
// (/api/rhm-config, the 11-slot whitelist). Every READ is registry-truth off an EXISTING endpoint
// (/api/rhm-config · /api/personas · /api/roles · /api/voice · /api/voice/paths · /api/capabilities). NO new
// config endpoint, NO parallel config state — it reads the SAME cfg/personas/now/fleet the RhmChat gear +
// Toolbar dial read (single source, rule 3). The legacy RhmChat gear stays working (forbidden file); this
...
// The mode is SWITCHABLE here (changeMode — it takes live: presence dot + mode line move). The off/suggest/auto
// AUTO-DETECT toggle renders its LIVE value (E2) — but READ-ONLY with an honest "env-set" note, because
// MODE_AUTODETECT has no runtime setter (no /api/rhm-config slot, env-resolved at __init__): per rule 4 / the
// G-19 precedent we render it UNAVAILABLE-to-edit, NEVER as a control that silently no-ops. (Flagged loud as a
// grown gap: add MODE_AUTODETECT to set_rhm_config + a route — suite.py/bridge.py, out of this lane's files.)
...
const SECTIONS: { id: 'brain' | 'modes' | 'voice' | 'roles' | 'composition'; icon: string; name: string; blurb: string }[] = [
  { id: 'brain', icon: '◈', name: 'Brain & persona', blurb: 'the model that thinks · the voice that speaks' },
...
export function Settings() {
  ...
  return (
    // the full-viewport modal scrim + panel (Workshop pattern). data-ui-ref is deliberately bare: ui://settings
    // is not yet a registered corpus address (the corpus is a file-disjoint lane) — flagged as a grown gap.
    <div className="settings-scrim" onClick={() => setSettingsOpen(false)}>
      <div className="settings-panel" onClick={e => e.stopPropagation()} role="dialog" aria-label="Settings">
        {/* the masthead — the surface's title + a live "where am I" cue + the close */}
        <div className="settings-masthead">
          <SectionHead tag="the company · control" aside={
            <button className="b ghost sm" onClick={() => setSettingsOpen(false)} title="close settings">✕ close</button>
...
// — BRAIN & PERSONA — the conscious model + the switchable cast. WRITE: setCfgSlot('model') · switchPersona.
function BrainSection({ cfg, chatModels, personas, setCfgSlot, switchPersona }: any) {
  return (
    <div className="settings-section" data-ui-ref="ui://settings/brain">
      <SectionHead tag="conscious role">The brain</SectionHead>
      <Surface tone="sig">
        <label className="set-field">
          <span className="set-label">Model <span className="muted">· the model the RHM thinks with</span></span>
          <select className="set-select" value={cfg?.model || ''} onChange={e => setCfgSlot('model', e.target.value)}>
            <option value="">default ({cfg?.model || 'fabric default'})</option>
...
// — VOICE — the ear (STT), the engine (TTS), the path (pipeline/s2s). WRITE: setCfgSlot('stt'|'tts_engine'|
// 'voice_path'|'voice_enabled'). s2s renders UNAVAILABLE (G-19), never as working.
function VoiceSection({ cfg, voiceStatus, voicePaths, setCfgSlot }: any) {
      ...
      <Surface>
        ...
        <label className="set-field">
          ...
          {sttIds.length === 0
            ...
            : <select className="set-select" value={cfg?.stt || ''} onChange={e => setCfgSlot('stt', e.target.value)}>
