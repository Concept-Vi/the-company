...
- `hooks/` — SessionStart + other Claude Code hook scripts wired via `.claude/settings.json`.
  First file: `cc_registry_freshness_check.sh` (Mirror-Registry LANE-REFRESH, F-FIX-13 / C-REF-1):
  compares the live binary version to `store/claude-code.version_stamp`; warns STALE if mismatched;
  silent on current; non-blocking (exits 0 always). A NEW pattern — no prior template in ops/hooks/.
