...
class Suite:
    ...
    def resolve_scope(self, ui_addr: str) -> dict:
        ...
        # ② CUTOVER (2026-07-02, Tim-authorized): the join now comes from THE LEDGER — derived inside
        # every deterministic build (ops/ledger_build.py PASS C: binds-ui → calls-endpoint →
        # serves-endpoint → powered-by edges), via the ONE shared function runtime/scope.resolve_scope
        # (the same fn the MCP face calls: one function, two faces). Contract unchanged: 5 keys, sorted,
        # S0 raise on malformed, empty scope = DENY-ALL, stale+breadcrumb when the source is unreachable.
        # The pre-ledger sidecar read (design/_system/code-symbols.json — a hand-made seed frozen at 71
        # registered addresses, lossy code://<stem> ids, 3 weeks stale at cutover) is COMMENTED OUT below
        # per Tim's no-fallback rule: inert + legible + recoverable, with no influence on the answer.
        # To re-derive the ledger join: .venv/bin/python ops/ledger_build.py --all --load
        from runtime.scope import resolve_scope as _ledger_resolve_scope
        return _ledger_resolve_scope(ui_addr)
        # --- RETIRED sidecar path (pre-2026-07-02) — kept for legibility, not for use ---
        # import json as _j
        # from contracts.ui_info import parse_ui_address
