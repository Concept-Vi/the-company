...
**Seam:** uses [[store — constitution]] (C1/C4), operates on C3 records, runs `compile` (C5), resolves context (C6), calls [[fabric — constitution]].
...
RHM, and the self-modification path, the one object [[mcp_face — constitution]] and the
bridge both speak to. The live capability list lives in [[Company Map]] — traverse there
