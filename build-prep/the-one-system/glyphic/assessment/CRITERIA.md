# [RECONCILED 2026-07-03 — FOLDED INTO THE CANONICAL PLAN, no longer a plan of record]
> This assessment was the drift: it superseded the original triad (`../CRITERIA.md` / GUIDE / SYNTHESIS /
> ROADMAP) instead of folding into it — Tim's correction. It stays as a build-log of what the W-loop did.
> Where each strand now lives:
> - W0 ✔ kept (truth/hygiene) · W1 ports ✔ kept as machinery (disposition: ../CRITERIA.md AMENDMENT A4)
> - W1 verbs + W2 placement/ordinal = DRIFT → redone under AMENDMENTS A2/A3/A4 (ROADMAP PHASE RECONCILE)
> - W2 read-out coverage ✔ kept · W3 (writer) / W4 (company wires) → already in ROADMAP's LIVE-INSTRUMENT groups
> - W5 "octagon decision" was a FALSE decision (nothing has one meaning — A1/A4); the register pass = A5's live-tune
> The plan of record is `../CRITERIA.md` (+ its AMENDMENTS) · `../GUIDE.md` · `../SYNTHESIS.md` (Round 7) · `../ROADMAP.md`.

# COMPLETION CRITERIA — the glyphic build (two-faced: FUNCTION + FORM; verified by USE)

> Each criterion is checkable. A group is DONE when every row passes BY USE (not by code-read).
> Laws standing over all rows: loud-fail · derive-never-place · additive on shared surfaces ·
> board-announce ops touches · copies-not-edits from counterpart · the STANDARD (product-level).

## W0 · TRUTH & HYGIENE
- [x] `core/cv-nodes.d.ts`: "glyphgraph" in CVGraphType union; line/direction/routing/lineColor declared
      on CVGraphEdge. VERIFY: tsc-parse or a JSDoc-consumer check; grep confirms no other consumer breaks.
- [x] REGROUNDING + THE-GENERATIVE-LANGUAGE carry §1 staleness corrections (spacing-resolved, G1-rescope,
      reactflow-dissolved, layout-jump-half-solved, counterpart-live). VERIFY: read-back.
- [x] ops/services.json chat-4b-fp8 gpu_util 0.9→0.44 (its own documented fitted value), board-announced.
      VERIFY: `company status` shows the fitter accepting the interaction loadout again.

## W1 · THE FURNITURE PORT (all: additive files in claude-ds; provenance header naming the source commit)
- [x] `assets/icons/cv-organisms.js` — organisms.js ported; ~30 `--dna-*` vars re-pointed to claude-ds
      tokens. VERIFY BY USE: a harness page renders mesh + hubNetwork + cascade + iconStrip re-tinting
      with the live tokens; zero raw hex (design-lint pass).
- [x] `core/cv-address.js` — the span/nested-fraction resolver (from resolve.mjs + address.json): spans,
      derived addresses, LCA-on-address-tree. VERIFY: the ported proofs pass (11/11 equivalents) in node.
- [x] `core/cv-arc.js` + `assets/icons/glyph-arc-seed.js` — the arc resolver (from resolve-sequence.mjs +
      sequence.json), renamed for conversation-growth events. VERIFY: 7/7-equivalent proofs pass.
- [x] `cv-edges.js` gains the `verb` facet (transport/gather(in|out)/descend/orbit) + the seam law,
      additive to `kind`. VERIFY: resolve({kind,verb}) returns composed facets; existing 4 kinds unchanged.
- [ ] Minted-glyph records adopt the co-located `voice` block convention (schema documented in
      LANGUAGE.md). VERIFY: the 12 language-family facets carry voice blocks; the read-out uses them.

## W2 · THE LIVING CANVAS
- [x] DiagramSolver's glyphgraph layout derives slot-position from the node's ADDRESS (cv-address spans),
      frozen on first placement; a drag writes an authored override. VERIFY BY USE: grow a graph 3→10
      nodes in the writer — ZERO existing node moves (screenshot before/after diff).
- [x] The growth animator: a graph mutation animates as the address-DIFF (changed = movers, LCA = the held
      anchor); no teleports. VERIFY BY USE in chrome: add/remove/retarget — motion reads as the change.
- [x] Fields: an `ordinal` CV_AXES axis (the ramp, monotonic-lightness invariant) paintable onto a
      glyphgraph's nodes by telling-order via the zone-wash mechanism. VERIFY: the writer shows order as
      temperature; toggling the field off returns neutral; no hex anywhere.
- [x] Read-out COVERAGE: every drawn relation appears in the spoken read-out (the sentence-coverage gap).
      VERIFY: 5 authored graphs — picture-clause count == sentence-clause count.

## W3 · THE CITIZEN WRITER (the product face — FORM half is Tim's eye at the end)
- [ ] The writer recomposed on containers.css (Band/Zone/Cluster; zones computed not boxed) + CanvasHeader
      + RefinePop + the Icons.jsx propose→adopt pattern; bespoke <style> reduced to page-specific only;
      zero hardcoded hex fallbacks (design-lint gate passes).
- [ ] glyphic-type.js sockets WIRED: node values/slots resolve through the registry's subscriptions
      (no hand-enumerated value lists). VERIFY: adding an axis value in the registry appears in the
      writer with no writer edit.
- [ ] Relation-teaching: unknown relation phrase → a teach affordance calling glyphic.author-relation →
      the edge kind exists + persists + re-writes. VERIFY BY USE (the 'signs' case ends connected).
- [ ] ONE icon-generation surface: writer's makeIcon (role-backed) becomes the shared engine;
      Icons.jsx + foundry consume it (or explicitly defer to it); the three-way fork is closed.
- [ ] Production shell: no unpkg/CDN dev builds; served precompiled (vite path) same-origin to the
      bridge; loads offline-tolerant. VERIFY: network tab shows zero CDN; CSP-clean.
- [ ] REACHABLE: the writer is in the Studio (canvas entry) AND the gallery (@dsCard), boot-checked.
- [ ] FORM gate: whole-screen review at desktop + phone; then Tim's recognition pass.

## W4 · THE COMPANY WIRES (each additive; ping the ledger thread if near its overnight lanes)
- [ ] modes/glyphgraph.py + a services.json combo (extends interaction; consent axis set), file-discovered,
      board-announced. VERIFY: set_mode('glyphgraph') round-trips; `company status` shows the combo.
- [ ] Burst-at-pause: glyph extract roles fired via run_swarm at utterance boundaries. VERIFY: a 3-utterance
      transcript produces 3 bursts; think=False confirmed structural.
- [ ] voice.transcript emit at the STT-return seam → /api/stream SSE. VERIFY: `curl /api/stream` shows the
      event while dictating; the browser writer receives it same-origin.
- [ ] Reindex route: POST /api/glyph/reindex (emit corpus + rebuild glyph_meaning). VERIFY: mint a symbol
      in the writer → reindex → it resolves by meaning without a shell command.
- [ ] The persistence ASK filed on the board (ledger.assertion vs own table) + answered/actioned with the
      ledger session. VERIFY: a saved glyphgraph has an ADDRESS that resolve_address dispatches.

## W5 · TIM-GATED (rides his availability; blocks nothing above)
- [ ] The register pass: ≥3 real chains read aloud to Tim's ear; wording corrected live via the author API.
- [ ] The octagon decision (three tables → his call). 
- [ ] The 4 fine spacing words.
