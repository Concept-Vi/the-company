...
### F3 — files were SILENTLY TRUNCATED — violates no-partial  ★ HIGH (Tim flagged: "should NEVER have been")
**162 files** exceed the 48,000-char cap (company 88 · counterpart 59 · claude-ds 15) — *(corrected from an earlier mis-count of 436, which used a wrong 24k threshold)*. *Why:* `ops/ledger_interpret_ollama.py:117-119` — `truncated = len(src) > MAX_FILE_CHARS (48000); src = src[:MAX_FILE_CHARS]` — a hard cut to the first 48k chars, with the cap labelled "kimi context." That cap is **far smaller than the models' real windows** (kimi-cloud and opus handle hundreds of thousands of tokens), so it was unnecessary for almost every file. Two populations were hit:
- **Real source/docs that MUST be read whole:** `runtime/suite.py` (875KB), `runtime/bridge.py` (270KB), `LINEAGE.md` (408KB), `OPERATOR-SURFACE-LOOP.md` (408KB), `application-composition-operational-mode.md` (368KB), `_ds_bundle.js` (1.34MB).
- **Generated data dumps that shouldn't be deep-interpreted at all:** `migration-pending/wizard-run-1/embed.jsonl` (45MB!), `.ndjson` scan rows, test fixtures, `.temp/pgdelta` catalog JSON, `candidates.json`, `code-edges.json`, `addresses.json`.

The accuracy audit confirms the *descriptions* are honest (the model hedged the unseen tail) — but an honest *partial* read still violates the no-partial law: 162 files have incomplete knowledge. *Fix (NEVER truncate):* (a) raise the cap to the model's real window so the vast majority read whole; (b) **chunk-and-stitch** any file still over it so even suite.py / the big docs are fully covered; (c) **classify pure data-dumps** and either exclude from interpretation or describe-as-data explicitly — never silent-truncate; (d) persist a `truncated` flag + window (currently NOT recorded — the ledger can't even tell which reads were partial); (e) re-interpret the affected real-source files at full length.

### F7 — dead/duplicate columns (defined, never filled)  ★ LOW (schema hygiene)
Seven columns are 0% filled: `inputs`, `outputs`, `kind`, `purpose_doc`, `stores_touched` (never prompted/extracted), and `model`, `prompt_version` (DUPLICATES of the live `interp_model` / `interp_prompt_version`, which are 100%). *Fix:* drop the duplicates; decide drop-vs-populate for the rest (F4 covers inputs/outputs). Clean schema = honest schema.
