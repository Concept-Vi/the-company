...
## ⚠ EVIDENCE STATUS — read before building on anything here (the discipline this rework is FOR)
This whole document is **PROVISIONAL pending Phase 0**. It is the output of six subagents reading source + docs — that is **research-inference, not machine-verification.** Reading the vLLM source is strong; computing a consequence and running it on the box are different things. Classify before relying:
- **OBSERVED-in-source (strong, but still confirm by a real boot):** `--kv-cache-memory-bytes` skips the −KV subtraction (gpu_worker.py:366-384); the `request_memory` startup gate reads gpu_util unconditionally (utils.py:408-421); the crash-loop logs (gpu_util 0.35, requested<weights); `VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS` default + our `=0` override; priority is dropped by the transport allowlist (fabric/transport.py:44).
- **INFERRED — computed, NOT run on the live engine (do NOT build on these until Phase 0 confirms):** the 1056-block "silent concurrency cap" + the 2-8× under-count (arithmetic, not measured); the per-seq 34 MiB (contaminated upward by activation — explicitly undercut by our own rigor agent); the ~39% enforce-eager tax (an external benchmark, not our box); fp8-KV tool-call safety; sleep-mode behaviour; KV-offload-on-WSL2.
- **The rule:** no INFERRED claim becomes load-bearing — in a message to Tim or in built code — until Phase 0 turns it OBSERVED-on-our-box. Phase 0 is not "a phase"; it is the conversion of this doc from believed to known.
- **A live instance of the failure mode this guards against:** commit `0045363` already wired `auto_gpu_util` with `per_seq_mb=34` — a number our own rigor agent then showed is contaminated/possibly non-linear. It's dormant (the FP8 still uses its static gpu_util), so harmless now, but Phase 1 MUST re-measure it clean before it goes live. Ship-confidently-then-undercut is the exact pattern to stop.
