...
> Status (2026-06-21): ALL 6 archetypes CONCRETE + VERIFIED — decision-card (built, proven) + graph · selector · instrument · diagram · spatial-material (contracts authored, each reuses archetype.schema via allOf, validated: render_kind const + required slots enforced). DNA renders each to its contract (her artefact-organism family sits UNDER render_kind as render-details — no new contract field, confirmed). The EDGE-BINDING is LOCKED (DNA 2026-06-21): an explicit `edges` slot, host-resolved to [{from,to,kind?}], never renderer-resolved — shared across the relational family (graph·diagram). Composition keeps ENRICHING these as the faces exercise them (not done at sufficiency — the contracts grow toward what the interface becomes). The shared laws (operator-legibility · DNA-token-built · resolver-placed · navigable-not-text · native dual-device) live in archetype.schema; each archetype adds its SLOT-SHAPE + TAKE + grounding.
