...
- **Three comment/annotation stores (added 2026-06-26 — see `CONSOLIDATION-THREE-AXES.md` §1):** the same act *leave a comment/mark* lands in three disconnected, differently-keyed stores by surface — board comment (`cc_board.comment`, `board://`, `noticeboard/*.md`) vs canvas annotation (`/api/annotate`→`ingest_comment`, `ui://`, `annotations.jsonl`, twin-training) vs surface mark (`/api/territory/write`→`mark`, `decision://`, marks store). Neither product front-end writes the board's polymorphic comment primitive (which already takes any scheme). Fix = converge front-ends onto `cc_board.comment`; folds into the store-union.
