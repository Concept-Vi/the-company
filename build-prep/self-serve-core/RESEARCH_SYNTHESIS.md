...
- **C1 · Search hierarchy (foundation).** Add session ⊂ project ⊂ cross-project SCOPING to Company search: a `project`/`session_ids` filter on `session_search` + `list_agent_sessions(project=…)`; supervisor stamps `project` on spawned sessions (close the live-session gap); a clean MCP `session_search` tool exposing the 3 scopes. Reuse recollection's project/session_ids filter shape (don't parallel it). Storage already carries `project` — this is scoping, not a rebuild.
...
- Is the cross-project search Company-owned (extend `session_search`) or recollection-owned (its recall already has project/session_ids) with Company calling it? (Both have an index; avoid two parallel cross-project searches.)
