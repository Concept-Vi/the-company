...
**Phase:** the commission has TWO halves — (A) the OPERATOR SURFACE and (B) the RIGHT-HAND-MAN as the always-present GUIDE. Half A's flagship (the DECISION SURFACE prove-on-one) is FUNCTION-proven end-to-end on the real card both viewports (render → host → Ask → real-button take → flips pending→decided → re-renders decided; the write-error fail-loud seam driven too); its full ROOT pass waits on DNA's option-affordance FORM fix (routed). ★ CORRECTION (2026-06-18, advisor-caught): half B — the RHM-as-GUIDE — had NOT been driven to the bar; the V was a mute verb-tool, not a teacher. Now building it: the FIRST SLICE is in (commit da3e870) — the V introduces itself at first contact + the fan names its aim; fresh-eyes confirms a stranger now understands the V is their guide. NEXT layer: the verb-fan legibility on touch (composition's ratified V-contract — routed, t-1781766012).
