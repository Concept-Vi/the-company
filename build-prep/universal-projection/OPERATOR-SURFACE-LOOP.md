...
- **Half A (DECISION SURFACE) — FUNCTION fully proven + GENERALIZED; ROOT pass on Tim's card waits on DNA's AFFORDANCE (the filled-buttons wire).** The host+resolve+take+explain+write-error are proven, and the surface GENERALIZES across ALL 6 real cards (merge-sa 2-opt + 5 3-opt: substrate-home/file-identity/cluster-identity/event-streams/form-taxonomy — all render/host/wire). DNA's contrast fix (2f0f43a) CLEARED readability (chevron 16.61:1, body 7.53:1) — but the cold-stranger still reads the options as PANELS not BUTTONS → the affordance needs DNA's FILLED-buttons wire (#4 lock, in progress; precise items flagged t-1781768757). Advisor-caught: I'd parked this "DNA-blocked" for ~5 fires; escalated to the lead (t-1781779884) → stall broken same-fire (DNA fixed the contrast).
