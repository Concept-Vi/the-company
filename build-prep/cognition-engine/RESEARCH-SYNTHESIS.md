...
---
# ROUND 3 — the 5-adversary review (the verified findings + the store rules)
**Converged (multi-adversary):** cascade RUNNER net-new (coherence_actions.py:5-8 in-code) · cloud routing net-new (run_role pins RESIDENT_BASE_URL) · marks≠finding-shape (finding={kind,address,route,state,source,owner,ts}, keyed by address; run-1 marks not findable → generalize) · lane-cut breaks (fabric/transport+client, vector_index — no owner; fs_store wrongly banned) · GUIDE over-sold reuse (criteria honest) · registry-bar = file-discovery+create_* not projection (cognition_info projects code-dicts) · field_types internal contradiction (widen-grammar, not a registry).
**No-regression (foundations adversary):** C7.3 the live-animated CRAFT-SURFACE DROPPED (Tim-flagged 2026-06-07) · G8 mis-stated/absent · C7.4 under-specified. Rest genuinely carried. Inspection-summary FAITHFUL (clean).
**System-fit (uncovered adversary):** the RHM-duplication (render = same shape as guided-review organ → EXTEND not parallel; = the pillar-relationship) · lineage absent (cross-session corroboration + inversion-finder need it; sequencing trap) · dropped disciplines (map-vs-reduce/effort-routing/resume-safety) · cascade can't encode human/Tim tiers · no ingestion path · 700-char-heads dodged the loop.
**The STORE rules (abide-by — from store/AGENTS.md):** portability = the ADDRESS never changes (a new backend implements the Resolver Protocol C4 + the C1 grammar; Supabase-later). Nevers: mutate cas:// · backend-above-Protocol · DB on /mnt/c · lose-versions · chat-turn-without-source. fcntl cross-process locks + fsync + ext4 + provenance. → fs_store IS editable (space-keyed vectors done right through the grammar+Protocol), abiding these.
**Cleared correctly-static (don't over-correct):** floor verbs · closed grammars (RULE_OPS/SCHEMES/DESTINATION_KINDS) · MODE_REGISTRY (the drift-fix) · the ActionRegistry-as-declarative · SlotBudget-reads-live.
