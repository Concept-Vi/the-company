...
## 2026-06-09 — the 4.5h CRITICAL-REVIEW landed (grounded, by-use). Corrects + confirms:
- ★ SURFACE IS REAL, NOT dev-stuff (prior pass wrong): objectively on the design system (280 corpus-token decls, Fraunces+Plex Mono), guided tour genuinely LEADS (spotlight + 1/3→2/3 + at-altitude WHAT/CAN-DO/HOW narration), click+talk at altitude, B1 streams. ~6-7 criteria met BOTH faces by-use (A1/A2/C2/D1/G1/H2/B2). Scorecard: build-prep/guided-review-surface/CRITICAL-REVIEW.md (agent updated in place).
- ★ TWO REAL BUGS (the priorities):
  1. F2 MAKE-OR-BREAK broken at the seam (CONFIRMED by-use): comment writes the annotation store; generate_mockup reads .feedback jsonl → "no actionable feedback". The store-mismatch I diagnosed.
  2. tldraw editor chrome BLEEDS through the review surface (toolbar/shape-tools/MADE-WITH-TLDRAW badge) — studio overlay doesnt occlude the tldraw UI (likely z-index/stacking, like the settings/view-switch occlusions). Biggest remaining feel-leak.
- DECISION (F2, dev-call per no-needs-tim-gating): generate reads the ANNOTATION STORE (the shared truth; the .feedback jsonl is retired per Review.tsx:14). PLAN: (a) extract bridge _CORPUS_META (code dict, bridge.py:92) → declared design/_system/corpus-meta.json (registry-is-truth; bridge + engine both read) so the engine can map mockup→ui:// address WITHOUT importing bridge; (b) generate_mockup gathers annotations_for(address) (replicate fs_store read of <store>/annotations.jsonl filtered by address) at the mockup base address; (c) descendant-gather (element-level comments, D3) = follow-on. Engine half = python-verifiable (no chrome); full loop + tldraw occlusion = need chrome (DOWN this session).
- NOT blind-building the F2 multi-file refactor in a no-chrome fire (would repeat shipping-broken-unverified). NEXT (chrome up): F2 store-unification + tldraw occlusion, both verified by use.

## 2026-06-09 fire — Q2 DONE (the F2 prerequisite, verified WITHOUT chrome)
- 1a0bfc1: extracted bridge _CORPUS_META hardcoded dict → design/_system/corpus-meta.json registry (no-hardcoding law — a violation I had flagged in scope Q2). Bridge loads it; JSON list unpacks like the old 4-tuple → zero consumer change. VERIFIED: /api/corpus items BYTE-IDENTICAL before/after (the complete data-contract check, no chrome needed), corpus_acceptance 23/suite_corpus_relations 23 pass, bridge parses, corpus-meta.json valid (22 entries).
- WHY it matters: the generate engine (runtime/generate_mockup.py, mine) can now read corpus-meta.json to map mockup→ui:// address WITHOUT importing bridge — the missing piece for Q3 (generate gathers a mockup's annotations at its address) → closes the F2 make-or-break comment→generate loop.
- NEXT (Q3, engine-half verifiable WITHOUT chrome): generate_mockup reads corpus-meta map + annotations_for(address) from <store>/annotations.jsonl, so a studio comment feeds generate. Python-testable (seed annotation → generate finds it). Then Q1 full-loop + P2 tldraw-occlusion need chrome.
- drift red is cognition's (run_index_incremental_acceptance unregistered), not mine.

## 2026-06-09 fire — Q3 DONE: comment→generate make-or-break ENGINE-HALF fixed + verified (no chrome)
- cfb60a7: generate_mockup now gathers feedback from the SHARED ANNOTATION store (where the studio comment writes via /api/annotate) — not the retired .feedback jsonl. gather_feedback = .feedback ∪ annotations_for(mockup address). Maps mockup→ui:// via the Q2 corpus-meta registry (no bridge import). Prefix-match (address == base OR base+"/...") gathers the mockups SURFACE + DESCENDANT element addresses → BOTH levels (whole + element, Tim not-either/or).
- VERIFIED BY USE (no chrome — engine is python): generate_mockup_acceptance ALL GREEN incl. new [Q3] test — seeded whole-mockup + element annotations → both gathered, unrelated surface excluded, generate actionable_feedback_count=2, comment carried into instruction_core, plan-safe. The "no actionable feedback" break Tim/the review hit is FIXED on the engine side.
- ★ F2 MAKE-OR-BREAK status: engine-half ✅ (comment→generate connects, by-use). REMAINING for the full loop: the FE click-path (comment in studio → click generate → see proposal) needs CHROME to verify end-to-end — chrome MCP down. The result-card FE already renders (critical-review confirmed). So the loop is wired; only the in-browser end-to-end confirm is pending chrome.
- Q2 (corpus-meta registry) + Q3 (annotation gather) both done+verified without chrome. NEXT (needs chrome): Q1 full-loop by-use + R1 element-deixis click-test + P2 tldraw occlusion.

## 2026-06-09 fire — F2 make-or-break BACKEND fully verified by-use (route-level, no chrome)
- /api/mockup-generate calls generate_for_mockup(mockup) plainly → auto-inherits the Q3 annotation gather (reads the live store). No route change needed.
...
## 2026-06-09 — V-B clicked-element comprehension DONE+verified (no chrome) + GROUP V captured
- Captured GROUP V (96d8fac): V-B (now), V-A (the cognition-corpus registry-generation chain — full pipeline: extract→ground→map[run_items]→reduce[cluster-dedup]→confirm[jury]→operator-propose, context/allocation/outputs), V-C (A+B self-completing), V-D (substrate-retrieval placeholder — Tim's RHM-button→RHM-docs idea, address-as-junction).
- V-B BUILT+VERIFIED (a7e440c): the registry-is-truth fix for "RHM said not registered". FE (StudioKit+useAppController): NO more fake ui:// minting — registered element→real dossier; un-registered→nearest REGISTERED ancestor + ships the element's actual content (text+html+tag) in focus.pointed_element. suite _chat_context: additive POINTED ELEMENT block + teaches registered-returns-vs-pointed-element. VERIFIED BY USE (no chrome): block injects-with-markup when pointed / absent when none / mockup still injected / framing teaches both. suite.py window claimed+released, cognition flagged.
- NEXT: chrome confirm the FE actually ships pointed_element on a real click (the FE→focus path; the injection is proven). Then V-A is the big one — loop-prep it as its own cognition-corpus pipeline (cross-lane w/ cognition's engine).

## 2026-06-09 fire — V-B FE→focus path VERIFIED by use (chrome); reply-wording = needs-tim
- Captured the live chat payload on a real un-registered click ("◐ layers" button): focus.selected = ["mockup://C1-inbox-desktop.html","ui://inbox"] — the mockup + the REAL registered ancestor (NO fake address), and focus.pointed_element = {text:"◐ layers", tag:"button", html:"<button class='btn'>◐ layers</button>"}. So the FE ships the real element, no fabrication. ✓
- Combined with the earlier by-use proof that suite _chat_context injects the POINTED ELEMENT block + the registered-vs-pointed framing → the full V-B mechanism is verified end-to-end (FE payload ✓ + injection ✓).
...
## 2026-06-09 — ★ FULL LOOP PREPARED (Tim: "full charge effort"): V-A registry-generation
- Loop-prep triad written + committed (157b13b): build-prep/registry-generation/ — RESEARCH-SYNTHESIS (grounded: ALL primitives exist on main — run_items/run_reduce[cluster]/run_role/save_cascade/screen_reader/parse.py/refcheck/governance-surface; gap = 23 mockups/~3000 elements/102 registered), COMPLETION-CRITERIA (RG1-11, two-faced, priority-ordered), IMPLEMENTATION-GUIDE (how + the chain-is-a-declared-cascade spine), COORDINATION (the [M]/[C] split).
- THE CHAIN: extract(parse.py) → ground(screen_reader) → MAP(run_items: register_element role) → REDUCE(run_reduce cluster-dedup+nest) → CONFIRM(jury+refcheck) → PROPOSE(governance batch) → Tim approves → WRITE-BACK(addresses.json+mockups+parse round-trip) → a once-dead element resolves richly. Declared as a re-runnable cascade (registry-is-truth).
- TWO PARALLEL LOOPS (Tim having cognition bolster): [M] mine = RG1 extract, RG7 cascade-spec, RG8 proposal-surface, RG9 write-back. [C] cognition = RG3 register_element role, RG6 confirm, engine gaps RG4/RG5. Meet at RG7 (cascade) + RG10 (whole-by-use = convergence proof). 2 seam contracts frozen: candidates.json schema (RG1) + register_element output_schema (RG3). Posted the split to cognition (157b13b).
- CRON: dedicated registry-generation loop 2ba276a3 @ 12,27,42,57 (staggered from guided-review f8cda8ab @ 5,20,35,50 + cognition/coherence). Session-only (terminal must stay open). This is now my PRIMARY build loop; guided-review-general (FE) is largely harvested (V-B/P2/R1/jargon/occlusions all landed+verified).
- STATUS of the guided-review surface itself: the dev-stuff leaks closed+verified (occlusions, jargon, any-element-select, tldraw-bleed); make-or-break backend proven (Q2+Q3+route); V-B clicked-element comprehension verified (FE payload + injection); needs-tim/needs-chrome: F2 full FE click-through, the felt reply, model-mismatch (Tim's 2 names), back-control (cognition).
- NEXT FIRES: the registry-generation cron builds RG1 (extract) first — it's ungated, mine, the seam-contract source. Then RG7/RG8/RG9 as cognition's RG3 role lands.

## 2026-06-09 — F2 make-or-break FULL CLICK-THROUGH in chrome (RHM pinned to kimi-k2.6:cloud, Tim-directed)
- RHM pinned to kimi-k2.6:cloud via /api/rhm-config (confirmed both ways) on temp bridge :8831 + vite :5231 (ollama serves it; vLLM :8000 was down).
...
## 2026-06-09 — V-A registry-generation COMMIT+RECORD (the-runner): 7/8 RG pieces landed by concurrent cognition; RG8 blocked-on-registration
- ROLE: I was the dedicated COMMIT+RECORD agent for the unified V-A build. The concurrent cognition session committed 7 of 8 RG pieces BEFORE my gate finished (the V-A ADOPTED handoff in MESSAGES.md). I VERIFIED their commits rather than re-committing — nothing duplicated.
- LANDED + VERIFIED (git show --name-only confirms ONLY-declared-files per commit; live discovery confirms function):
  · ff990fa — RG3 register_element.py + RG6 confirm_registration.py + roles/AGENTS.md (both reflections, count=2). Both roles discover live; check_dossier importable.
  · 690ab28 — RG1 parse.py + candidates.json (940 units) · RG7 cascade.json · RG9 registry_writeback.py · RG6-refcheck floor.
- RG8 (RegistryProposals.tsx) — BUILT-VERIFIED but BLOCKED-FOR-THIS-AGENT, kept in tree, NOT committed:
  · cognition explicitly HELD it for this (FE) lane.
  · Isolation-proven it adds 5 NEW orphans to the already-red ui_registry_acceptance suite (ui://registry/proposals + /approve + /approve-bar + /notice + /refresh, none in addresses.json). Cannot be made gate-clean without auto-mutating the real addresses.json (operator-only floor BARS this) or editing RG9's lane. Remediation = operator-gated RG9 write-back registering those 5 addresses on Tim's approve. Recorded as blocked-on-registration, no green-paint.
  · Still orphaned (mounted by no region) — mounting + the GET /api/registry/proposals + POST /api/registry/approve routes are the RG9/RG10 follow-on.
- GATE: 8 standing reds (consult, convergence_signoff, event_address, reachability, rhm_completion, twin_located_gold, ui_registry, wire_trigger) — ALL pre-existing cross-session reds, NONE RG-caused (ui_registry red WITHOUT my tsx on the 3 prior UI-lineage orphans; the other 7 reference no RG file). Other lanes' fix-board, do not block this record.
- FROZEN SEAM CONTRACTS (committed): (1) candidates.json {generated_from,count:940,filter,candidates[{mockup_file,selector,outerHTML,visible_text,tag,ancestor_address,ancestor_dossier,base_address,self_address,self_registered}]} (RG1); (2) register_element output_schema {address,represents,howto,capabilities,maps_to_feature,confidence} (RG3).
- REMAINING PROOF: RG10 whole-by-use — run the cascade → a once-dead element resolves richly after Tim approves. NEEDS-TIM: embedder GPU window for cluster-reduce; the engine seams (confirm→run_jury, {mockup}-keyed shared-context, cluster embed model); RG8 mount + backend routes + the FORM/feel of the batch-review walk.
