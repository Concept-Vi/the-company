...
- **CAND-10 ·★ THE AXES (dimensionality OPEN — corrected by wildcard's rigor-guard, do NOT over-claim)** — FIVE sources reached a four-part axis structure (composition state·lineage·order·frame / fork provenance·semantic·temporal·structural / wildcard T·Pi·A·C / FRD scale·temporal·orientation·influence / GLOSSARY four-root-axes). BUT four-ness ≠ convergence (wildcard 2026-06-15): per-axis semantic test shows only the **TIME axis maps cleanly 5-way (STRONG)**; the rest split into scale/hierarchy, state/provenance, relation clusters nobody carves the same. Likely a HIGHER-DIM object projected as different four-slices (hypothesis (b), fits Tim's multiple-entry-points law) rather than one four-grid (a). ★ ROUTE TO TIM AS A QUESTION: how many axes are there? Never silent-merge five into four. Decisive test: is the FRD equation's dimensionality FIXED or scale-relative (BRR hints scale-relative → (b))? See ROUND1_SYNTHESIS.md.
