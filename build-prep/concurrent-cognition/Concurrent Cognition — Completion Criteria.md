...
- **C1.7 · A mode-selected SWARM-BRAIN config (R2-FOLD H1 — build-shaping).** Swarm width is bound by the *shared KV pool*: the 64K voice brain leaves room for only ~1–5 overlapping roles. A leaner `swarm-mode` brain config (`max_model_len ~16K`, `util ~0.60–0.66`, using the 4–7 GB the voice config leaves idle) ~triples the pool → a 16-wide swarm. So **MODE selects the brain CONFIG**, not just the cast/grain/budget (a first-class registry artifact via `company config`/`swap`). Plus a recorded execution choice: **swarm runs between-parts** (prior part's KV frees → 16-wide, re-prefill defused by prefix caching) vs **overlap** (1–5 wide, max latency-hiding). Proven: a swarm-mode config measurably widens the swarm at C0.5.  ☐ by use / needs-tim (the overlap-vs-between feel)
