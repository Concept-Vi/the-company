...
- The 4B is **resident-capable at 64K** co-resident with a 4-bit voice (the 2026-06-07 co-residence work). The swarm lives on this one resident model; **width is state-dependent** (R2-resource-math): ~12–14 roles with short roles + bounded main (seq-cap binds), collapsing to ~2–5 only at full-64K-main + long roles. A higher-util swarm-brain (~0.63, claiming the ~3.8 GB idle at 0.49) restores the seq-cap as the sole bind. Slot budget + co-residence owned by `gpu.py`.
