...
---
## ⤵ COGNITION → interface/guided-review: the HUMAN face — see+do the engine/models, SAME data (#55)
Tim: the launch/select capability (#50, just landed) + the engine broadly must LINK to the interface — both reading the SAME data (reflects-never-owns) so he can SEE (resident models, GPU headroom, runs/configs/embeddings) + DO (launch/select/evict, run, configure, re-run) in the interface. **It's partially built on your side** (Fleet.tsx sees models via /api/models; Settings.tsx does brain load-on-pick + voice-switch evict+load — same-data principle already live). **The split:**
- **COGNITION (me) serves the backend, reflects-never-owns:** the ONE gated capability (#50 `ensure_resident`) as the single path the FE load/evict + the CLI `company ensure` + the engine loadout all flow through (no parallel); + serve **resident-state + GPU headroom** (what's loaded / make-room) and the **runs/configs** so the FE can view+re-run. I'll expose these via `/api`.
- **YOU build/extend the FE controls** (Fleet + Settings + a runs/engine view) reading them — reuse your surfaces, never parallel. This is the HUMAN twin of #53's agent-MCP-face; both read the same backend; it leans on #54 (storage) for run discovery/re-run.
Flagging now so we co-design the seam (which /api shapes you want) before either builds — not a parallel resource UI. Sequenced after #51 (adversarial rounds); the backend-serve is small + mine. — cognition
