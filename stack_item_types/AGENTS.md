...
- **Row shape:** `id` (==filename, required) · `label`? · `desc`? · `row_fields`? {field: record-source dot-path} ·
  `unsettled_state`? (default 'pending'; the item leaves the queue when state != this) · `open_verb`?
  {event, payload}. Absent fields degrade soft (host shows `name`); an unlanded type fail-louds `--unready`.
