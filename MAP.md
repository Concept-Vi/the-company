...
**The decision→implementation wire** (`runtime/implement.py` + `dispatch_decision`, Group W) closes *recorded decision → governed dispatch to Claude Code (`claude -p`, headless) → verify → result back → status=`implemented`* with no human re-prompt — reusing the `derived_from` bind (authorization), the event log (exactly-once, under a per-seq lock + the durable claim event + visibility), POLICY posture, and the separate `status` lane (closes without writing the operator `resolved` field). Only an `AUTO`-posture declared class auto-dispatches (`decision_build`); CONFIRM/SURFACE/LOCKED classes surface for the operator; the close is `guard("code_build")`-ed on the verification verdict; an empty declared scope is deny-all. It is OFF the MCP face — the RHM surfaces a build-intent, it never dispatches one. The unattended watcher/loop that calls it is the WIRE-LOOP seam (built separately).
