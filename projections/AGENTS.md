...
- **`mesh`** — agent-authored projection (created via the declarative-direct face). the MESH triangulation lens (2026-07-07, Tim's 'all the equipment and no mesh' directive): observation + triangulation records from the self
...
- **`common_knowledge`** — meaning · **code** · **embeds** · text. The fabric's COMMON-KNOWLEDGE index — recollection's comprehended built-things (sessions/projects/board/exchanges, addressed in contracts.address), embedded as the overlord interface's render-field; each unit carries its verification-state (the state/phase axis). The recollection↔projection seam (Option A: publish into this company space, pplx/dim-2560). See build-prep/front-interface/INTERFACE_BRIEF.md.
